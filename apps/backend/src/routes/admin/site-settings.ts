...
export async function adminSiteSettingsRoutes(app: FastifyInstance) {
  app.get('/api/v1/admin/site-settings', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const { data: row, error } = await supabaseAdmin
      .from('site_settings')
      .select(
        'template_id, theme_primary, theme_secondary1, theme_secondary2, cta_variant, cta_phone_label, cta_email_label, cta_submit_label, cta_success_message, lead_notification_email, lead_formspree_url'
      )
